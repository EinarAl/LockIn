...
router.post('/:courseId', asyncHandler(async (req: AuthRequest, res: Response) => {
  const { category, name, score, maxScore, date } = req.body
  const grade = await Grade.create({
    user: req.userId,
    course: req.params.courseId,
    category, name, score: Number(score), maxScore: Number(maxScore), date,
  })
...
router.get('/:courseId/analysis', asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/:courseId/what-if', asyncHandler(async (req: AuthRequest, res: Response) => {
  const targetGrade = Number(req.body.targetGrade)
  if (isNaN(targetGrade) || targetGrade <= 0 || targetGrade > 100) {
    return res.status(400).json({ error: 'Enter a valid target grade (1-100)' })
  }
  const course = await Course.findOne({ _id: req.params.courseId, user: req.userId })
  if (!course) return res.status(404).json({ error: 'Course not found' })
  ...
  const totalWeight = categories.reduce((sum, cat) => sum + cat.weight, 0)
  if (totalWeight <= 0) {
    return res.json({ currentGrade: null, neededOnRemaining: null, remainingWeight: 0, message: 'No grade categories defined.' })
  }
  ...
  const currentPercent = (currentGrade / totalWeight) * 100

  const remainingWeight = Math.max(0, totalWeight - weightSoFar)
  let needed = null
  if (remainingWeight > 0) {
    needed = (targetGrade - currentPercent) * totalWeight / remainingWeight
